(function($) {
  ...
  // Swiper Default Slider JS
    var mainlSlider = new Swiper('.hero-slider-container', {
      ...
      allowTouchMove: false, // Disable touch/swipe since only one slide
      // Removed pagination since only one slide is needed
    });
  ...
  function parallaxScroll() {
    ...
    if ($parallaxImages.length && window.innerWidth > 768) {
      varWindow.on('scroll', function() {
        ...
        $parallaxImages.each(function() {
          ...
          var windowHeight = varWindow.height();
          
          // Check if element is in viewport
          if (containerOffset < scrollTop + windowHeight && containerOffset + containerHeight > scrollTop) {
            // Calculate parallax movement with more dramatic effect
            var scrollPercent = (scrollTop - containerOffset + windowHeight) / (windowHeight + containerHeight);
            var maxMovement = containerHeight * 0.25; // 25% of container height for more movement
            var yPos = (scrollPercent - 0.5) * maxMovement * 2; // Double the effect
            
            // Clamp the movement to prevent cutting
